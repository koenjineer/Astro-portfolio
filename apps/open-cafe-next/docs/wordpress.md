...
| 店舗 | 3 | タイトルと同じ日本語（全角スペース入り） | 全件あり（地図の画像 map_kichijoji〜） |
...
- メニュー・ギフト・店舗のslugは日本語。詳細ページが無いのでURLには使わない。メニュー・店舗はslug順の並びが読みにくい（ギフトは投稿日順）
...
ギフトは、`public/` に画像の実物が無くてもビルドが止まる（`lib/images.server.ts`）。
