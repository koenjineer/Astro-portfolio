interface ArrowRightIconProps {
  /** サイズと位置は使う側で指定する（w-5 h-[14px] など） */
  className?: string;
}

/**
 * 右向き矢印アイコン（Figma: icon-arrow-right）。
 * ルールセットのホバー仕様が「文字とアイコン色を反転」のため、塗りをcurrentColorにして
 * 親の文字色に追従させる。色違いのSVGファイルを増やさずに反転できる。
 */
export function ArrowRightIcon({ className }: ArrowRightIconProps) {
  return (
    <svg
      width={20}
      height={14}
      viewBox="0 0 20 14"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
      focusable="false"
      className={className}
    >
      <path
        d="M12.7544 0.268056C12.4001 0.632613 12.4001 1.23778 12.7475 1.60962L16.9712 6.04992H0.896144C0.402917 6.04992 0 6.47281 0 6.99777C0 7.52273 0.402917 7.94562 0.896144 7.94562H16.9642L12.7405 12.3859C12.3932 12.7578 12.4001 13.3556 12.7475 13.7275C13.1018 14.092 13.6645 14.092 14.0188 13.7202L19.743 7.66856V7.66856C19.8194 7.58106 19.8819 7.48628 19.9305 7.36962C19.9792 7.25296 20 7.12901 20 7.00506C20 6.75717 19.9097 6.52385 19.743 6.34157L14.0188 0.289933C13.6784 -0.0892101 13.1087 -0.0965004 12.7544 0.268056Z"
        fill="currentColor"
      />
    </svg>
  );
}
