...
const SHOPS_QUERY = `
  {
    shops(first: ${SHOPS_FETCH_LIMIT}, where: {orderby: {field: SLUG, order: ASC}}) {
      nodes {
        title
        slug
        shopFields {
...
function toShop(node: ShopNode): Shop {
  ...
  return {
    ...
    seats: fields.count,
    mapImageSrc: requireLocalImageSrc(
      node.featuredImage,
      SHOP_IMAGE_DIR,
      `店舗（${node.title}）`
    ),
...
export const getShops = cache(async (): Promise<Shop[]> => {
  ...
  const shops = data.shops.nodes.map(toShop);
