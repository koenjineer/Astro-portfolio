...
### 資料ダウンロード・お問い合わせの下調べ（2026-09-06）

デザインには**完了ページが両方に存在する**（例：資料ダウンロード完了 Figma node-id=14592-5331。
フォームの位置にお礼文とダウンロードリンクが入り、パンくずが「トップ > 資料ダウンロード >
資料ダウンロード完了」になる独立した1ページ）。`/download` と `/download/thanks`、
`/contact` と `/contact/thanks` の別ページとして静的に書き出す。
...
- 配る資料PDFの実体をどうするか。静的サイトなのでURLを知ればフォームを通さず落とせる
  （「フォーム経由の人だけに配る」はこの構成では実現できない＝エンジニア相談）
- 完了ページの「メールアドレス宛にもリンクを送付しております」の一文。指している自動返信メールは
  Professional以上の**有料機能**で無料枠では実際には送られない。文言を消すか残すか
  （表示と実際の動きを合わせる意味では消すのを勧めた）
