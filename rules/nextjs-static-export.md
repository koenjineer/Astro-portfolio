...
カンプからの素材の書き出しは `/rules/design-to-code.md`。
...
**Next.jsはビルドのたびにビルドIDとチャンク名を作り直す**（`_next/static/<ビルドID>/` と
`chunks/<ハッシュ>.js`）。この2つはHTMLの中にも書かれるので、何も直していなくても `diff -r` は
差分を出す。**この2つを伏せてから比べる**（PythonなどでビルドIDを置換し、
`chunks/xxx.js` を `chunks/HASH.js` にそろえてから、`out/**/*.html` を突き合わせる）。
`_next/static/` の中身は見なくてよい（HTMLが一致していれば見た目は同じ）。
