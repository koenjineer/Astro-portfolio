...
|---|---|---|---|
| ボタン・矢印つき（`button-utility-pc`） | 色が変わり、矢印が少し右に移動 | 白地・`main` 枠 → `main` 塗り・白文字 | ホーム（`app/_components/UtilityButton.tsx`） |
...
| お知らせの行（`news-link-pc`、日付＋タイトル＋右矢印） | 色が変わり、矢印が少し右に移動 | 文字が `main` に | ホーム（`app/_components/HomeNewsLink.tsx`） |
| ホームの診療案内カード（写真＋白文字） | フィルターが濃くなり、画像が少し拡大、枠線を表示 | 膜が `black/20` → `black/40`、画像1.1倍、内側10pxに白2pxの枠線（角丸20px） | ホーム（`app/_components/MedicalGuideCard.tsx`。枠線は擬似要素で出す） |
| トップのブログカード（`top-blog-card-pc`） | 文字の色が変化し、サムネイル画像が少し拡大 | 同上＋サムネイル1.1倍（ホバーの絵の 330/300） | ホーム（`app/_components/TopBlogCard.tsx`） |
...
ホームのメインビジュアルの矢印は `BackToTop` に合わせて `main` で塗り山形を白に、ドットは今の1つ以外が `main` に変わる。
